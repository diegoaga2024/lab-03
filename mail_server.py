...
def add_mail(mail_entry: Dict[str, str]) -> str:
    """
    Adds a new mail entry to the mail list and saves the updated mail list to the json file
    
    Args:
    	mail_entry (dict): A dictionary representing the new mail entry
...
def delete_mail(mail_id: str) -> bool:
    """
    Deletes a mail entry from the mail list and saves the updated mail list to the json file
    
    Args:
    	mail_id(str): The id of the mail entry to delete
    	
    Returns:
    	bool: True if mail was deleted, False otherwise
    """
    mail = load_mail()
...
def get_mail(mail_id: str) -> Optional[Dict[str, str]]:
    ...
    Returns:
    	A dictionary representing the mail entry if it exists, None otherwise
    """
    mail = load_mail()
    for entry in mail:
        if entry['id'] == mail_id:
...
def get_inbox(recipient: str) -> List[Dict[str, str]]:
    """
    Gets all the mail entries for a recipient from the mail list
    Args:
    	recipient (str): The recipient of the mail entries to get
    	
    Returns:
    	list: A list of dictionaries representing the mail entries for the recipient
    """
    mail = load_mail()
    inbox = []
    for entry in mail:
...
def get_sent(sender: str) -> List[Dict[str, str]]:
    """
    Retrieves the sent mail entries of a specific sender.
    
    Args:
    	sender(str): The email address of the sender to retrieve sent mail entries for.
    
    Returns:
    	list: A list of dictionaries reprenting the mail entries for the sender.
    """
    mail = load_mail()
    sent = []
    for entry in mail:
...
# HINT: start with somthing like this:
